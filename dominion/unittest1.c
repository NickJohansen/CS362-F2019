...
int main (int argc, char** argv){
	...
	printf("Begin Testing do_minion():\n");
//
...
//
	...
	cardEffect(minion, c1, c2, c3, &game1, hand, bonasu);
	if(game1.coins == 12){
		printf("Test for getting 2 coins Passed [PASS]\n");
	}
	else{
		printf("Test for getting 2 coins Failed [FAIL]\n");
	}
	if(game1.handCount[0] == 0){
		printf("Test for discarding card Passed [PASS]\n");
	}
	else{
		printf("Test for discarding card Failed [FAIL]\n");
	}
...
// verify that your assertspass, for the bugs your asserts may fail
	...
	game2.hand[1][0] = adventurer;
	game2.hand[1][1] = adventurer;
	game2.hand[1][2] = adventurer;
	game2.hand[1][3] = adventurer;
	game2.hand[1][4] = adventurer;
	game2.hand[1][5] = adventurer;
	game2.handCount[0] = 1; // set any other variables
	game2.handCount[1] = 6;
	cardEffect(minion, c1, c2, c3, &game2, hand, bonasu);
	if(game2.handCount[0] == 4){
		printf("Test for getting 4 cards Passed [PASS]\n");
	}
	else{
		printf("Test for getting 4 coins Failed [FAIL]\n");
	}
	if(game2.handCount[1] == 4){
		printf("Test for other player 4 cards Passed [PASS]\n");
	}
	else{
		printf("Test for other player 4 cards Failed [FAIL]\n");
	}
