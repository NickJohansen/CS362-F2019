	...
	if (choice1 > 0) { //Boolean true or going to discard an estate
		...
		while (card_not_discarded) {
			if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
				...
				//BUG ADDED: Changed the number of coins added from 4 to 2, which is half of the correct amount.
		...
		if (supplyCount(estate, state) > 0) {
			...
			//BUG ADDED: Commented out the call to gainCard function, so the current player
			//does not correctly gain an estate card like they should.
...
int minionCardEffect(int choice1, int choice2, struct gameState *state, int handPos)
{
	...
	//BUG ADDED: Changed the number of coins gained from 2 to 4, twice the correct amount.
	if (choice1)
	{
	...
	else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
	{
		...
		//draw 4
		//BUG ADDED: Changed the number of cards drawn from 4 to 2, half of the correct amount.
		for (i = 0; i < 2; i++)
		{
...
int ambassadorCardEffect(int choice1, int choice2, struct gameState *state, int handPos)
{
	...
	//BUG ADDED: Commented out the call to discardCard, so the played card remains in the player's hand.
//	discardCard(handPos, currentPlayer, state, 0);
...
int tributeCardEffect(struct gameState *state)
{
	...
	if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
		if (state->deckCount[nextPlayer] > 0) {
			...
			//BUG ADDED: Changed the deckCount of nextPlayer to increase rather than decrease,
			//causing problems with the deck count of the players.
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
		mineCardEffect(choice1, choice2, state, handPos);
    ...
    case minion:
		minionCardEffect(choice1, choice2, state, handPos);
